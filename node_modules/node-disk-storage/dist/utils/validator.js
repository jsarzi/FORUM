...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "is-any-type", "zlib", "human-size", "./next", "./error", "./matchProperty"], factory);
    }
    ...
    const validatorKeyVal = (...data) => {
        ...
        return res.includes(true) ? next_1.next() : res;
    };
    ...
    const sizeValidator = (options, value) => {
        let toJSON;
        if (!options.compress) {
            toJSON = zlib_1.default.gzipSync(Buffer.from(JSON.stringify({ data: value }))).toString('utf-8');
        }
        else {
            toJSON = JSON.stringify({ data: value });
        }
        const bodySize = human_size_1.default(toJSON.length);
        const sizeMatch = [];
        let MB = '';
    ...
    const propertyValidator = (options) => {
        if (is_any_type_1.assert.isBoolean(matchProperty_1.matchProperty(options)))
            return true;
        else
